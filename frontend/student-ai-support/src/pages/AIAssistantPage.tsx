...
export const AIAssistantPage: React.FC = () => {
  ...
  const filteredSessions = sessions.filter(
    (s) =>
      toSafeString(s.title).toLowerCase().includes(chatSearch.toLowerCase()) ||
      toSafeString(s.category).toLowerCase().includes(chatSearch.toLowerCase()),
  );

  const todaySessions = filteredSessions.filter(
    (s) =>
      toSafeString(s.updatedAt).includes("ago") ||
      toSafeString(s.updatedAt).includes("Just now") ||
      toSafeString(s.updatedAt).includes("Today"),
  );
  const yesterdaySessions = filteredSessions.filter(
    (s) =>
      toSafeString(s.updatedAt).toLowerCase().includes("yesterday") ||
      toSafeString(s.updatedAt).includes("1 day"),
  );
  const olderSessions = filteredSessions.filter(
    (s) => !todaySessions.includes(s) && !yesterdaySessions.includes(s),
  );
          ...
          <aside className="w-[300px] border-r border-[#c2c7d1] bg-white flex flex-col h-full shrink-0">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-4">
              ...
              {todaySessions.length > 0 && (
                <div>
                  <p className="text-[10px] uppercase font-bold text-[#727780] px-2 mb-1 tracking-wider">
                    Today
                  </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#42474f]">
                    Active RAG pipeline connected to OpenSearch Open-Text
                    Repository
                  </p>
                </div>
              </div>
              ...
              {loadingMessages ? (
                <div className="h-full flex items-center justify-center text-[#0f4c81] gap-2">
                  ...
                  <span className="text-sm font-semibold">
                    Loading RAG conversation history...
                  </span>
                </div>
              ) : messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-3">
                  ...
                  <h3 className="font-headline font-bold text-xl text-[#00355f]">
                    How can I assist you today?
                  </h3>
                  <p className="text-xs text-[#42474f]">
                    Ask any question regarding course registration, GPA
                    criteria, hostel guidelines, or tuition fee schedules.
                  </p>
                </div>
              ) : (
                messages.map((msg) => (
                          ...
                          <div className="bg-white border border-[#c2c7d1] rounded-2xl p-5 shadow-xs text-sm text-[#0b1c30] space-y-4">
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-[#c2c7d1]/60 text-xs text-[#42474f]">
                                ...
                                <button
                                  onClick={() =>
                                    sendMessage({
                                      ...
                                      content:
                                        "Please expand on this answer with additional details.",
                                    })
                                  }
                                  className="flex items-center gap-1 px-2.5 py-1 hover:bg-[#eff4ff] rounded-md transition-colors cursor-pointer"
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  onClick={() =>
                                    handleFeedback(msg.id, "unhelpful")
                                  }
                                  className={`p-1.5 rounded-md cursor-pointer transition-colors ${
                ...
                <div className="flex gap-4 max-w-4xl mx-auto">
                  ...
                  <div className="bg-white border border-[#c2c7d1] rounded-2xl p-4 shadow-xs flex items-center gap-3">
                    ...
                    <span className="text-xs font-semibold text-[#00355f]">
                      Searching OpenSearch vector index & invoking
                      Bedrock/Gemini...
                    </span>
                  </div>
                </div>
            ...
            <div className="bg-white border-t border-[#c2c7d1] p-4 shrink-0 shadow-lg">
              <div className="max-w-4xl mx-auto space-y-2">
                ...
                <form
                  ...
                  className="relative flex items-center bg-[#eff4ff] border border-[#c2c7d1] rounded-2xl p-2 focus-within:ring-2 focus-within:ring-[#0f4c81]/30 focus-within:border-[#0f4c81] transition-all"
                >
                  <label
                    className="p-2 text-[#42474f] hover:bg-[#dce9ff] rounded-xl cursor-pointer transition-colors"
                ...
                <p className="text-[10px] text-center text-[#727780]">
                  AI Assistant uses RAG vector search over official
                  institutional documents. Always verify regulatory details with
